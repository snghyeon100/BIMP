...
# -*- coding: utf-8 -*-
"""
train_fam.py  —  Training script for DSS_fam model.
(Familiar/Novel Dual-Head Pooling:
...
from models.DSS_fam import DSS_Base as DSS
...
def get_cmd():
    ...
    parser.add_argument("-m", "--model",   default="DSS_fam", type=str)
...
def main():
    ...
    for lr, l2_reg, UB_ratio, UI_ratio, BI_ratio, emb_size, num_layers, c_lambda, c_temp in \
        ...
        # ------------------------------------------------------------------
        # Post-training: β 최종값 출력
        # ------------------------------------------------------------------
        beta_final = torch.sigmoid(model.beta).item()
        lam_final  = torch.clamp(model.lambda_ubui, min=0.0).item()

        print("\n" + "=" * 60)
        print("Training finished.")
        print(f"  β (familiar weight) = {beta_final:.4f}")
        if beta_final > 0.5:
            print(f"  → 모델이 Familiar(아는 아이템) 쪽을 더 선호 ({beta_final:.1%})")
        else:
            print(f"  → 모델이 Novel(새로운 아이템) 쪽을 더 선호 ({1-beta_final:.1%})")
        ...
        with open(log_path, "a") as f:
            f.write(f"\n[Post-training] beta(familiar_weight)={beta_final:.4f}, lambda={lam_final:.4f}\n")
