...
# -*- coding: utf-8 -*-
"""
DSS_Base.py  —  Baseline (no PE, spmm-optimized).

s_base(u, b) = UI_u · mean_item(b) + UB_u · UB_b

Optimization: bi_agg_graph [NB, NI] sparse (values=1/bundle_size)
  → torch.spmm(bi_agg, UI_i) computes all bundle item-means in one call.
  → bundle_emb pre-computed in get_embeddings and passed via embs dict.
  → compute_scores: simple lookup  (no 3D tensor)
  → _score_all_bundles: torch.mm  (no scatter_add, no 3D tensor)
"""
...
class DSS_Base_bonus(nn.Module):
    """
    Baseline DSS: s_base only, BPR loss.
    ...
    def __init__(self, conf, raw_graph, bundle_info=None):
        ...
        i = torch.LongTensor(np.vstack((alpha_coo.row, alpha_coo.col)))
    ...
    def _build_bi_agg_graph(self):
        ...
        torch.spmm(bi_agg_graph, UI_i) → [NB, d]  item-mean per bundle.
        """
    ...
    # ------------------------------------------------------------------
    # Embedding extraction — pre-computes bundle_emb via spmm
    # ------------------------------------------------------------------
    ...
    def get_embeddings(self, test=False):
        ...
        # Pre-compute item-mean embedding for ALL bundles — O(NI*d) spmm
        ...
        return {
            ...
            "bundle_emb": bundle_emb,  # [N_b, d]  item-mean repr (NEW)
        }
    ...
    def compute_scores(self, embs, users, bundles):
        ...
        users_1d = users.view(-1)
        eu_UI = embs["UI_users"][users_1d].unsqueeze(1).expand(-1, n_b, -1).reshape(M, d)
        eu_UB = embs["UB_users"][users_1d].unsqueeze(1).expand(-1, n_b, -1).reshape(M, d)

        b_emb = embs["bundle_emb"][flat_b]     # [M, d]  item-mean
        ...
        # --------------------------------------------------
        # Add Bonus Score: gamma_bonus * log1p(mean_alpha)
        # --------------------------------------------------
        alpha_sparse = self.alpha_pt.index_select(0, users_1d) # [bs, NI]
        alpha_u = alpha_sparse.to_dense() # [bs, NI]
        ...
        # Expand alpha_u for batch indexing
        alpha_c = alpha_u.unsqueeze(1).expand(bs, n_b, -1).gather(-1, items_all) # [bs, n_b, n_t]
        alpha_c = alpha_c.masked_fill(~mask_all, 0.0)
        n_valid = mask_all.float().sum(-1).clamp(min=1)
        mean_alpha = alpha_c.sum(-1) / n_valid # [bs, n_b]
        
        bonus = self.gamma_bonus * torch.log1p(mean_alpha)
    ...
    # ------------------------------------------------------------------
    # Evaluation — pure torch.mm, no 3D tensor
    # ------------------------------------------------------------------
    ...
    def _score_all_bundles(self, embs, users):
        ...
        # s_base = UI_u @ bundle_emb.T + UB_u @ UB_b.T
        s_base = torch.mm(UI_u, bundle_emb.t()) + torch.mm(UB_u, UB_b.t()) # [bs, NB]

        # --------------------------------------------------
        # Compute mean_alpha_all efficiently for evaluation
        # alpha_pt: [NU, NI], bi_agg_graph.T: [NI, NB]
        # Product: [NU, NB] where (u, b) = sum_i(alpha_{ui}/size_b) = mean_alpha(u, b)
        # Because we're only calculating for specific users, we take the slice
        # --------------------------------------------------
        alpha_sparse = self.alpha_pt.index_select(0, users) # [bs, NI]
        
        # Multiply by sparse bi_agg_graph.T
        # We need to convert alpha_sparse to dense to use spmm efficiently
        # Since torch.spmm needs sparse matrix as the first argument: spmm(NIxNB.T, bsxNI.T).T -> [bs, NB]
        bi_agg_sparse = self.bi_agg_graph # [NB, NI]
        alpha_dense = alpha_sparse.to_dense().t() # [NI, bs]
        mean_alpha = torch.spmm(bi_agg_sparse, alpha_dense).t() # [bs, NB]
