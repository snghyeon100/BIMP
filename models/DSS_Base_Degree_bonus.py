...
class DSS_Base_Degree_bonus(nn.Module):
    """
    ...
    def _compute_alpha(self, users_1d):
        # 파이토치 sparse GPU 텐서에서 users_1d 추출 후 dense 변환
    ...
    def compute_scores(self, embs, users, bundles):
        ...
        n_t       = items_all.shape[-1]                      # 아이템 최대 개수
        mask_all  = (self.bundle_items[bundles] >= 0)        # [bs, n_b, n_t]
        
        flat_items = items_all.reshape(-1) # [bs * n_b * n_t]
        unique_t, inv_idx = torch.unique(flat_items, return_inverse=True)

        # alpha 계산: [bs, n_unique]
        alpha_cmp = self._compute_alpha_items(users_1d, unique_t)  # [bs, n_unique]

        # inv_idx 이용해서 alpha_all 매핑: [bs * n_b * n_t] -> [bs, n_b, n_t]
        # inv_idx_reshaped = inv_idx.reshape(bs, n_b, n_t)  # shape mismatch.
        # 주의: alpha_cmp는 [bs, n_unique] 이고 각 element는 유저마다 다름.
        pos_all = inv_idx.reshape(bs, n_b, n_t)  # [bs, n_b, n_t]
        
        # [bs, n_b, n_t] 텐서를 flatten해서 alpha_cmp.gather에 써야함.
        # alpha_cmp: [bs, n_unique]. pos_all: [bs, n_b, n_t].
        # dim=1 이므로 n_b*n_t 로 합쳐서 gather
        alpha_all = alpha_cmp.gather(1, pos_all.view(bs, -1)).view(bs, n_b, n_t)
        
        alpha_all_raw = alpha_all.masked_fill(~mask_all, 0.0)
        
        # softmax 계산: [bs, n_b, n_t]
        w_all = torch.softmax(alpha_all.masked_fill(~mask_all, -1e9), dim=-1)

        # 아이템 임베딩: unique에서만 조회 (backward: 200개 scatter 대신 sgemm)
        n_unique = unique_t.shape[0]
        unique_iembs = UI_i[unique_t]                              # [n_unique, d]

        # w를 unique 공간에서 집계: [bs, n_b, n_unique]
        w_unique = torch.zeros(bs, n_b, n_unique, device=self.device)
        w_unique.scatter_add_(2, inv_idx.view(bs, n_b, n_t), w_all)
    ...
    def _score_all_bundles(self, embs, users):
        ...
        for u_start in range(0, bs, USER_CHUNK):
            ...
            # α_u,i: [uc, NI] — 유저 청크별로 한 번씩만 계산 (OOM 방지)
            ...
            for b_start in range(0, NB, BUNDLE_CHUNK):
                ...
                w = torch.softmax(alpha_c_soft, dim=-1)                  # [uc, C, T]

                # item embeddings [C, T, d]
                iembs  = UI_i[items_c.view(-1)].view(C, T, d)            # [C, T, d]
                # (C, uc, T) @ (C, T, d) -> (C, uc, d) -> (uc, C, d)
                v_star = torch.bmm(w.transpose(0, 1), iembs).transpose(0, 1)
                ...
                il_chunks.append((UI_u.unsqueeze(1) * v_star).sum(-1) + bonus)   # [uc, C]
