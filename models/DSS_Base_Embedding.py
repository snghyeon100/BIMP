...
# -*- coding: utf-8 -*-
"""
DSS_Base_Embedding.py  —  Personalized Weighted Bundle Embedding.
...
# ---------------------------------------------------------------------------
# DSS_Base_Embedding Model
# ---------------------------------------------------------------------------

class DSS_Base(nn.Module):
    """
    DSS Baseline with Personalized Weighted Bundle Embedding (Embedding Inner Product).

    bundle representation:
        DSS_Base_Embedding: v*_u,b = weighted mean using (e_UI_u · e_UI_i)
    ...
    def compute_scores(self, embs, users, bundles):
        ...
        # (기존의 Count 대신 유저-아이템 임베딩 내적으로 부드럽게 평가)
        # [bs, d] @ [d, n_unique] -> [bs, n_unique]
        alpha_cmp = torch.matmul(UI_u, unique_iembs.T)
    ...
    def _score_all_bundles(self, embs, users):
        ...
        for u_start in range(0, bs, USER_CHUNK):
            ...
            # α_u,i: [uc, NI] 
            # (기존 Count 대신 유저-아이템 임베딩 내적으로 부드럽게 계산)
            alpha_u = torch.matmul(UI_u, UI_i.t())
