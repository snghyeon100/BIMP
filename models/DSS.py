...
class MAB(nn.Module):
    ...
    def forward(self, X, Y, mask_y=None, return_attn=False):
        B, n_q, d = X.shape
        n_k       = Y.shape[1]
        h, hd     = self.h, self.hd
        ...
        attn_weights = attn  # save before dropout for inspection
        ...
        if return_attn:
            return out, attn_weights   # attn_weights: [B, h, n_q, n_k]
        return out
...
class DSS(nn.Module):
    ...
    def __init__(self, conf, raw_graph, bundle_info):
        ...
        # ------------------------------------------------------------------
        # §1.2 Bundle initial state:
        #   H_b^(0) = MLP_h([e_b^BI ; Pool({e_i^UI | i∈I_b})])
        #   Pool = mean of UI item embeddings via bi_agg_graph (spmm)
        #   MLP_h: Linear(2d→d) + ReLU + Linear(d→d)
        # ------------------------------------------------------------------
        self.mlp_bundle_init = nn.Sequential(
        ...
        # ------------------------------------------------------------------
        # §1.3 User query: q_u = LN(W_uiq·UI_u)  (UI view only)
        # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # §3.4  2-hop B→I expansion:  A'_BI = A_BI + beta_bi * S_topk
    #        Used ONLY in _bundle_to_item neighbor lookup.
    #        LightGCN graphs / I→B path are NEVER touched.
    # ------------------------------------------------------------------

    def _build_expanded_bi_adj(self):
        """
        Build expanded item→bundle CSR adjacency (A'_BI^T) for BIMP B→I step.

        S = norm_row(A_BU) @ norm_row(A_UI)   [NB, NI]
        S_topk: per-bundle top-K of S (sparse)
        ...
        # ---- row-normalised A_BU and A_UI (scipy sparse) ----
        A_UB = self.ub_graph            # [NU, NB]
        A_UI = self.ui_graph            # [NU, NI]
        A_BI = self.bi_graph            # [NB, NI]
        A_BU = A_UB.T.tocsr()          # [NB, NU]

        bu_deg = np.array(A_BU.sum(axis=1)).ravel() + 1e-8
        ui_deg = np.array(A_UI.sum(axis=1)).ravel() + 1e-8
        A_BU_n = sp.diags(1.0 / bu_deg) @ A_BU   # [NB, NU]
        A_UI_n = sp.diags(1.0 / ui_deg) @ A_UI   # [NU, NI]

        # ---- 2-hop score matrix S = A_BU_n @ A_UI_n  [NB, NI] (sparse) ----
        S = (A_BU_n @ A_UI_n).tocsr()            # [NB, NI]

        # ---- Top-K per bundle row ----
        S_coo = S.tocoo()
        rows, cols, vals = S_coo.row, S_coo.col, S_coo.data

        # Sort within each row and keep top-K
        # Use a dict: row -> list of (val, col)
        from collections import defaultdict
        row_dict = defaultdict(list)
        for r, c, v in zip(rows, cols, vals):
            row_dict[int(r)].append((float(v), int(c)))

        tk_rows, tk_cols, tk_vals = [], [], []
        for b, pairs in row_dict.items():
            pairs.sort(key=lambda x: -x[0])
            for v, c in pairs[:K_topk]:
                tk_rows.append(b)
                tk_cols.append(c)
                tk_vals.append(v)

        S_topk = sp.coo_matrix(
            (np.array(tk_vals, dtype=np.float32),
             (np.array(tk_rows, dtype=np.int32),
              np.array(tk_cols, dtype=np.int32))),
            shape=(NB, NI)).tocsr()

        # ---- A'_BI = A_BI + beta_bi * S_topk ----
        A_BI_f    = A_BI.astype(np.float32)
        A_BI_exp  = (A_BI_f + beta_bi * S_topk).tocsr()

        # ---- A'_BI^T → CSR for item→bundle lookup ----
        A_BI_exp_T = A_BI_exp.T.tocsr()           # [NI, NB]

        ptr = A_BI_exp_T.indptr.astype(np.int64)  # [NI+1]
        adj = A_BI_exp_T.indices.astype(np.int64) # [nnz]

        self.register_buffer("item_ptr_exp",
            torch.tensor(ptr, dtype=torch.long, device=self.device))
        self.register_buffer("item_adj_exp",
            torch.tensor(adj, dtype=torch.long, device=self.device))

        # ---- Diagnostic log ----
        nnz_orig  = int(A_BI.nnz)
        nnz_exp   = int(A_BI_exp.nnz)
        nnz_stopk = int(S_topk.nnz)

        # Average N(i) before and after expansion
        n_orig = np.diff(A_BI.T.tocsr().indptr)  # per-item degree in A_BI^T
        n_new  = np.diff(ptr)
        print(f"[BI-Expand] beta_bi={beta_bi}, K_bi_topk={K_topk}")
        print(f"[BI-Expand] nnz(S_topk)={nnz_stopk:,}  "
              f"nnz(A_BI)={nnz_orig:,} → nnz(A'_BI)={nnz_exp:,} "
              f"(+{nnz_exp-nnz_orig:,})")
        print(f"[BI-Expand] avg |N(i)| orig={n_orig.mean():.2f} "
              f"→ expanded={n_new.mean():.2f}")

    # ------------------------------------------------------------------
    # §3.3  Sparse BI aggregation matrix (for s_base fast-path)
    # ------------------------------------------------------------------

    def _build_bi_agg_graph(self):
        """
        Build a sparse [NB, NI] matrix where entry (b, i) = 1/bundle_size[b].
        torch.spmm(bi_agg_graph, UI_i) → [NB, d]  item-mean per bundle in one call.
        Replaces the 3D tensor [M, n_t, d] pattern used in _compute_sbase.
        """
        NB, NI = self.num_bundles, self.num_items
        bi_np  = self.bundle_items.cpu().numpy()   # [NB, n_t]
        n_t    = bi_np.shape[1]

        b_idx  = np.repeat(np.arange(NB), n_t)    # [NB*n_t]
        i_idx  = bi_np.reshape(-1)                 # [NB*n_t]
        valid  = i_idx >= 0
        ...
        self.bi_agg_graph = to_tensor(bi_agg).to(self.device)   # [NB, NI] sparse
    ...
    def _bimp_bundle_init(self, BI_b, UI_i):
        """
        H_b^(0) = MLP_h([e_b^BI ; Pool({e_i^UI | i∈I_b})])  →  [N_b, d]
        Pool = mean of UI item embeddings (bi_agg_graph spmm)
        """
        pool = torch.spmm(self.bi_agg_graph, UI_i)          # [N_b, d]
        x    = torch.cat([BI_b, pool], dim=-1)               # [N_b, 2d]
        return self.mlp_bundle_init(x)                       # [N_b, d]

    def _user_query(self, UI_u):
        """q_u = LN(W_uiq·UI_u)  →  same shape as input  (UI view only)"""
        return self.ln_user_q(self.W_uiq(UI_u))
    ...
    def _bundle_to_item(self, X_i_loc, H_b_loc, H_b_init_full,
        ...
        #    B→I uses expanded adj (A'_BI^T): item_ptr_exp / item_adj_exp
        # ------------------------------------------------------------------
        ptr_s = self.item_ptr_exp[items_uniq]
        ptr_e = self.item_ptr_exp[items_uniq + 1]
        raw_deg = ptr_e - ptr_s
        ...
        flat_bundles = self.item_adj_exp[flat_abs]       # [total_edges] global bundle ids (expanded)
        ...
        if getattr(self, '_log_extra_once', True):
            ...
            # Compare raw N(i): original A_BI^T vs expanded A'_BI^T
            orig_deg = (self.item_ptr[items_uniq + 1] - self.item_ptr[items_uniq]).float().mean().item()
            exp_deg  = raw_deg.float().mean().item()
            # print(f"[B2I ctx] avg raw |N(i)|: orig={orig_deg:.1f} → expanded={exp_deg:.1f} "
            #       f"(+{exp_deg-orig_deg:.1f})")
            # print(f"[B2I ctx] mean |N_act|={n_act_mean:.1f}  "
            #       f"|N_extra|={n_ext_mean:.1f}  |N_used|={n_use_mean:.1f}")
    ...
    def _bimp_forward(self, embs, bundles_uniq, items_uniq, train):
        ...
        H_b_init_full = self._bimp_bundle_init(BI_b, UI_i)  # [N_b, d]
        ...
        # ------------------------------------------------------------------
        # I→B token trimming: keep top-K items per bundle by importance score
        #   score(i) = _bi_item_deg[i] + _ui_item_deg[i]
        #   (BI deg = "cross-bundle representative",  UI deg = "user popularity")
        #   Invalid (pad) positions are set to -inf so they never win.
        # ------------------------------------------------------------------
        K_i2b = int(self.conf.get("max_item_K_i2b", 40))
        n_t_orig = items_loc_cs.shape[1]
        if n_t_orig > K_i2b:
            # importance score per slot [U, n_t]
            bi_deg_slot = self._bi_item_deg[items_loc_cs]          # [U, n_t]
            ui_deg_slot = self._ui_item_deg[items_loc_cs]          # [U, n_t]
            score = (bi_deg_slot + ui_deg_slot).masked_fill(~valid_loc, float('-inf'))

            # top-K indices [U, K_i2b] (preserves original relative order is fine;
            # topk gives largest values — pads at -inf are never selected)
            topk_idx = score.topk(K_i2b, dim=1, largest=True, sorted=False).indices  # [U, K_i2b]

            # Gather trimmed views of all loop-invariant tensors
            items_loc_cs  = items_loc_cs.gather(1, topk_idx)                          # [U, K_i2b]
            items_loc_idx = items_loc_idx.gather(1, topk_idx)                         # [U, K_i2b]
            valid_loc     = valid_loc.gather(1, topk_idx)                             # [U, K_i2b]
            pe_loc        = pe_loc.gather(1, topk_idx.unsqueeze(-1).expand(-1, -1, self.emb_size))  # [U, K_i2b, d]
    ...
    def _compute_sbase(self, UI_u_flat, UB_u_flat, UI_i, UB_b, flat_b):
        ...
        Returns s_base : [M]
        """
        # spmm: [NB, NI] x [NI, d] → [NB, d]  item-mean per bundle (no 3D tensor)
        mean_all  = torch.spmm(self.bi_agg_graph, UI_i)   # [NB, d]
        mean_emb  = mean_all[flat_b]                       # [M,  d]
    ...
    def compute_scores(self, embs, users, bundles, return_components=False):
        ...
        # ---- Stage2 ----
        q_u   = self._user_query(eu_UI)                  # [M, d]
    ...
    def _score_all_bundles(self, embs, users, return_comps=False):
        ...
        # spmm: one call instead of BC_b chunk loop
        mean_items_all = torch.spmm(self.bi_agg_graph, UI_i)   # [NB, d]

        scores = (torch.mm(UI_u_all, mean_items_all.t())
                  + torch.mm(UB_u_all, UB_b.t()))              # [bs, NB]
        ...
        if single_pass:
            ...
            for u0 in range(0, bs, UC):
                ...
                q_uc  = self._user_query(eu_UI)                        # [uc, d]
