...
# -*- coding: utf-8 -*-
"""
train_fam2.py  —  Training script for DSS_fam2 model.
(Stage 2: Personalized β_u = ui_deg / (ui_deg + ub_deg)
    v_trust  = softmax(+α) · e_i
    v_motive = softmax(−α) · e_i
    v*_u,b   = β_u · v_trust + (1−β_u) · v_motive   ← 유저별 고유 β
)
"""
...
from models.DSS_fam2 import DSS_Base as DSS
...
def get_cmd():
    ...
    parser.add_argument("-m", "--model",   default="DSS_fam2",   type=str)
...
def main():
    ...
    print("Baseline: Disabled Hard Negative Sampling (hard_neg_prob=0.0)")
    ...
    for lr, l2_reg, UB_ratio, UI_ratio, BI_ratio, emb_size, num_layers, c_lambda, c_temp in \
        ...
        # ------------------------------------------------------------------
        # Post-training: β_u distribution 출력
        # ------------------------------------------------------------------
        lam_final  = torch.clamp(model.lambda_ubui, min=0.0).item()
        beta_stats = model.user_beta.cpu()

        print("\n" + "=" * 60)
        print("Training finished. [Stage 2: Personalized β_u]")
        print(f"  β_u distribution:")
        print(f"    mean  = {beta_stats.mean():.4f}")
        print(f"    std   = {beta_stats.std():.4f}")
        print(f"    min   = {beta_stats.min():.4f}")
        print(f"    max   = {beta_stats.max():.4f}")
        print(f"  λ (UI/UB balance) = {lam_final:.4f}")
        print(f"Run: python analyze_degree.py -d {conf['dataset']} --ckpt {checkpoint_model_path}")
        print("=" * 60 + "\n")

        with open(log_path, "a") as f:
            f.write(f"\n[Post-training Stage2] beta_u mean={beta_stats.mean():.4f} "
                    f"std={beta_stats.std():.4f}, lambda={lam_final:.4f}\n")
